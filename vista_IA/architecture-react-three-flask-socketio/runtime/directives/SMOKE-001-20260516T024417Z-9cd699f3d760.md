...
- Generated at: `2026-05-16T02:44:17Z`
- Sprint: `10`
- Source hash: `9cd699f3d7606ea9e0b17debf9649ce586057784699322967cdbb6f08c4ed11d`
- Checkpoint: `none`

## Worker Instruction

```text
Task: SMOKE-001 - Create smoke evidence
...
Goal: Create one expected smoke evidence file through the control plane.
Exact deliverables:
...
- smoke-output.txt
Expected validation:
- python3 -B -c "from pathlib import Path; missing=[p for p in ['smoke-output.txt'] if not Path(p).is_file()]; assert not missing, missing"
- python3 -B -c "from pathlib import Path; assert Path('smoke-output.txt').is_file(), 'smoke-output.txt'"
...
- Missing required evidence before execution: smoke-output.txt
- Missing expected file: smoke-output.txt
Closure criteria:
