# Directive RECOVERY-001

- Generated at: `2026-05-16T02:44:22Z`
- Sprint: `10`
- Source hash: `e0bbbc878fb8688666c4bd8f17603bd10062f90959a7cc081a540a007e504c98`
- Checkpoint: `recovery-001-retry-0-recovery`

## Worker Instruction

```text
Task: RECOVERY-001 - Recover failed task through retry
...
Goal: Fail once, persist recovery, retry the same task, then complete it.
Exact deliverables:
...
- recovery/output.txt
Expected validation:
- python3 -B -c "from pathlib import Path; missing=[p for p in ['recovery/output.txt'] if not Path(p).is_file()]; assert not missing, missing"
- python3 -B -c "from pathlib import Path; text=Path('recovery/output.txt').read_text(encoding='utf-8'); assert 'recovered' in text, 'recovered'"
...
Starting checkpoint:
- recovery-001-retry-0-recovery
Known risks or blockers:
- Missing required evidence before execution: recovery/output.txt
- Missing expected file: recovery/output.txt
- Prior failure for active task: Missing expected evidence files: recovery/output.txt; Process exited with return code 7; Missing expected file: recovery/output.txt; Validation command failed with return code 1: python3 -B -c "from pathlib import Path; missing=[p for p in ['recovery/output.txt'] if not Path(p).is_file()]; assert not missing, missing"; Validation command failed with return code 1: python3 -B -c "from pathlib import Path; text=Path('recovery/output.txt').read_text(encoding='utf-8'); assert 'recovered' in text, 'recovered'"
Closure criteria:
