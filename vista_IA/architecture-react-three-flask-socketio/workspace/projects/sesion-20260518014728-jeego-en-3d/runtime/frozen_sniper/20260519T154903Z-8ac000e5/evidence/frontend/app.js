...
function boot() {
  ...
  setHudText(hud.mode, modeName);
  try {
    new DroneGame(modeName);
  } catch (error) {
    console.error(error);
    startCanvasFallback(modeName, error);
  }
}
...
function prepareFallbackCanvas() {
  const current = document.querySelector("#world");
...
function startCanvasFallback(modeName, error) {
  if (fallbackGame) return;
  const shell = document.querySelector("#app-shell");
  const canvas = prepareFallbackCanvas();
  if (!canvas) {
    setHudText(hud.event, `error render: ${error.message}`);
    return;
  }
  shell?.setAttribute("data-renderer", "canvas-fallback");
  setHudText(hud.event, `fallback canvas activo: ${error.message}`);
  fallbackGame = new CanvasFallbackGame(canvas, modeName, error);
  fallbackGame.start();
}
...
class CanvasFallbackGame {
  constructor(canvas, modeName, error) {
    this.canvas = canvas;
    this.ctx = canvas.getContext("2d");
    this.modeName = modeName;
    this.config = MODES[modeName] || MODES.build;
    this.rand = mulberry32(this.config.seed + 991);
    this.error = error;
    this.frameId = null;
    ...
    this.lastReward = 0;
    this.lastPenalty = 0;
    this.lastEvent = "fallback canvas activo";
    this.nextObstacleZ = 72;
    this.obstacles = [];
  ...
  start() {
    if (!this.ctx) {
      setHudText(hud.event, "fallback canvas no disponible");
      return;
    }
    this.bindModes();
    this.buildQBars();
  ...
  resetField(countEpisode = true) {
    if (countEpisode) this.episode += 1;
    this.distance = Math.max(0, this.distance + 22);
    this.altitude = 14;
    this.lateral = 0;
    this.verticalVelocity = 0;
    this.speed = this.config.baseSpeed;
    this.obstacles = [];
    this.nextObstacleZ = this.distance + 70;
    while (this.nextObstacleZ < this.distance + 760) {
      this.addObstacle();
    }
    this.lastEvent = countEpisode
      ? "fallback reinicio por colision"
      : "fallback canvas: vuelo autonomo iniciado";
  }

  addObstacle() {
    const dense = this.config.buildingDensity + this.config.treeDensity;
    const kindRoll = this.rand();
    const side = this.rand() > 0.5 ? 1 : -1;
    const centerLane = randRange(this.rand, -12, 12);
    const obstacle =
      kindRoll < 0.34
        ? {
            type: "edificio",
            x: side * randRange(this.rand, 30, 72),
            y: randRange(this.rand, 22, 44),
            z: this.nextObstacleZ,
            width: randRange(this.rand, 8, 16),
            height: randRange(this.rand, 24, 58),
            color: this.rand() > 0.5 ? "#30433f" : "#493a34"
          }
        : kindRoll < 0.68
          ? {
              type: "arbol",
              x: side * randRange(this.rand, 18, 28),
              y: randRange(this.rand, 4, 9),
              z: this.nextObstacleZ,
              width: randRange(this.rand, 3.8, 6.5),
              height: randRange(this.rand, 7, 14),
              color: "#2f7f4f"
            }
          : {
              type: "obstaculo",
              x: centerLane,
              y: randRange(this.rand, 10, 24),
              z: this.nextObstacleZ,
              width: randRange(this.rand, 5, 12),
              height: randRange(this.rand, 3, 9),
              color: "#f6b94b"
            };
    this.obstacles.push(obstacle);
    const spacing = clamp(58 - dense * 1.6, 24, 54);
    this.nextObstacleZ += randRange(this.rand, spacing, spacing + 34);
  }

  ensureObstacles() {
    this.obstacles = this.obstacles.filter((obstacle) => obstacle.z > this.distance - 80);
    while (this.nextObstacleZ < this.distance + 820) {
      this.addObstacle();
    }
  }
  ...
  update(time) {
    ...
    const scan = this.scanPath();
    const actionIndex = this.pickAction(scan);
    const action = ACTIONS[actionIndex];
    this.lastAction = actionIndex;

    this.lateral += action.steer * dt * 13;
    this.verticalVelocity += action.lift * dt * 7.5;
    this.speed += action.throttle * dt * 8.5;
    this.speed = clamp(this.speed, this.config.baseSpeed * 0.62, this.config.speedLimit);
    this.altitude += this.verticalVelocity * dt;
    this.verticalVelocity *= 0.92;
    this.altitude = clamp(this.altitude, 5, 42);
    this.lateral = clamp(this.lateral, -24, 24);
    this.distance += this.speed * dt;

    const reward = this.score(scan, actionIndex, dt);
    this.render(scan);
    this.updateHud(scan, reward.net, reward.penalty);
  ...
  scanPath() {
    ...
    for (const obstacle of this.obstacles) {
      const forward = obstacle.z - this.distance;
      if (forward < -5 || forward > range) continue;
      const xGap = Math.abs(obstacle.x - this.lateral) - obstacle.width * 0.5;
      const yGap = Math.abs(obstacle.y - this.altitude) - obstacle.height * 0.5;
      const inCorridor = xGap < 4.5 && yGap < 4.5;
      if (inCorridor && forward < minDistance) {
        minDistance = Math.max(0, forward);
        nearest = obstacle;
      }
    }

    const sidePressure = nearest ? clamp((nearest.x - this.lateral) / 22, -1, 1) : 0;
    const verticalPressure = nearest ? clamp((nearest.y - this.altitude) / 20, -1, 1) : 0;
    return {
      minDistance,
      nearest,
      sidePressure,
      verticalPressure,
      normalized: clamp(minDistance / range, 0, 1)
    };
  }

  pickAction(scan) {
    const danger = 1 - scan.normalized;
    this.lastQ = ACTIONS.map((action, index) => {
      let value = (index === ACTIONS.length - 1 ? 0.3 : 0) + this.rand() * 0.18;
      if (scan.nearest) {
        value += action.steer * -scan.sidePressure * danger * 3.2;
        value += action.lift * -scan.verticalPressure * danger * 2.4;
      }
      if (index === 4) value += scan.normalized * 0.8 - danger * 1.9;
      if (index === 5) value += danger * 1.1 - scan.normalized * 0.2;
      if (this.altitude < 8 && action.lift > 0) value += 1.4;
      if (this.altitude > 34 && action.lift < 0) value += 1.1;
      if (Math.abs(this.lateral) > 18 && action.steer === -Math.sign(this.lateral)) value += 1.2;
      return clamp(value, -4, 4);
    });
    return argmax(this.lastQ);
  }

  score(scan, actionIndex, dt) {
    let reward = this.speed * dt * 0.16 + (scan.minDistance > 24 ? 0.05 : 0);
    let penalty = Math.abs(this.lateral) * 0.01 + Math.abs(this.altitude - 16) * 0.006;
    if (scan.minDistance < 15) penalty += (15 - scan.minDistance) * 0.09;
    if (actionIndex === 4 && scan.minDistance < 22) penalty += 0.22;

    const collision = scan.nearest && scan.minDistance < 2.4;
    if (collision) {
      penalty += 10;
      this.lastEvent = `fallback impacto: penalidad ${penalty.toFixed(2)} por ${scan.nearest.type}`;
      this.resetField(true);
    } else if (scan.nearest && scan.minDistance > 36 && scan.minDistance < 42) {
      reward += 0.65;
      this.lastEvent = `fallback evasion correcta: recompensa +${reward.toFixed(2)}`;
    } else {
      this.lastEvent = "fallback canvas: vuelo autonomo visible";
    }

    const net = clamp(reward - penalty, -12, 4);
    this.lastReward = reward;
    this.lastPenalty = penalty;
    this.rewardHistory.push(net);
  ...
  projectPoint(x, y, z) {
    ...
    const forward = Math.max(1, z - this.distance);
    const depth = clamp(1 - forward / 820, 0, 1);
    const scale = 0.22 + depth * depth * 3.9;
    return {
      x: width * 0.5 + (x - this.lateral * 0.24) * scale * 9,
      y: horizon + depth * height * 0.62 - y * scale * 2.2,
      scale,
      depth
    };
  }
  ...
  render(scan) {
    ...
    sky.addColorStop(0, "#183348");
    sky.addColorStop(0.42, "#10201f");
    sky.addColorStop(1, "#070909");
    ctx.fillStyle = sky;
    ...
    ctx.strokeStyle = "rgba(155,166,157,0.56)";
    ctx.lineWidth = 2;
    for (const lane of [-0.16, 0, 0.16]) {
      ctx.beginPath();
      ctx.moveTo(width * (0.5 + lane * 0.36), horizon);
      ctx.lineTo(width * (0.5 + lane), height);
      ctx.stroke();
    }

    const visible = this.obstacles
      .filter((obstacle) => obstacle.z > this.distance && obstacle.z < this.distance + 820)
      .sort((a, b) => b.z - a.z);
  ...
  drawObstacle(obstacle) {
    const ctx = this.ctx;
    const p = this.projectPoint(obstacle.x, obstacle.y, obstacle.z);
    const width = Math.max(4, obstacle.width * p.scale * 7);
    const height = Math.max(6, obstacle.height * p.scale * 4.2);
    const x = p.x - width * 0.5;
    const y = p.y - height * 0.5;

    ctx.fillStyle = obstacle.color;
    if (obstacle.type === "arbol") {
      ctx.fillRect(p.x - p.scale * 4, p.y + height * 0.24, p.scale * 8, height * 0.45);
      ctx.beginPath();
      ctx.moveTo(p.x, y);
      ctx.lineTo(x - width * 0.1, p.y + height * 0.28);
      ctx.lineTo(x + width * 1.1, p.y + height * 0.28);
      ctx.closePath();
      ctx.fill();
      return;
    }
    ...
    ctx.strokeStyle = obstacle.type === "obstaculo" ? "#ffe3a0" : "rgba(255,255,255,0.16)";
    ctx.lineWidth = Math.max(1, p.scale * 1.3);
    ctx.strokeRect(x, y, width, height);

    if (obstacle.type === "edificio") {
      ctx.fillStyle = "rgba(245,231,176,0.72)";
      const rows = Math.max(2, Math.floor(height / 18));
      const cols = Math.max(2, Math.floor(width / 18));
      for (let row = 0; row < rows; row += 1) {
        for (let col = 0; col < cols; col += 1) {
          if ((row + col + Math.floor(obstacle.z)) % 3 === 0) continue;
          ctx.fillRect(x + 6 + col * 14, y + 8 + row * 16, 6, 8);
        }
      }
    }
  }
  ...
  drawDrone(scan) {
    ...
    const width = window.innerWidth;
    const height = window.innerHeight;
    const x = width * 0.5 + this.lateral * 2.5;
    const y = height * 0.57 - this.altitude * 1.3;
    const tilt = clamp(this.lateral / 24, -1, 1) * 10;
    ...
    for (const rotor of [
      ...
      ctx.beginPath();
      ctx.ellipse(rotor[0], rotor[1], 18, 5, performance.now() * 0.02, 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.restore();

    if (scan.nearest) {
      const target = this.projectPoint(scan.nearest.x, scan.nearest.y, scan.nearest.z);
  ...
  drawCanvasStatus() {
    ...
    ctx.fillStyle = "rgba(237,244,238,0.8)";
    ctx.fillText("CANVAS FALLBACK RENDER", 18, window.innerHeight - 92);
    ctx.restore();
  }

  updateHud(scan, netReward, penalty) {
    setHudText(hud.distance, `${Math.max(0, this.distance).toFixed(0)} m`);
    setHudText(hud.altitude, `${this.altitude.toFixed(1)} m`);
    setHudText(hud.speed, `${this.speed.toFixed(1)} m/s`);
    setHudText(hud.episode, `${this.episode}`);
    setHudText(hud.reward, `${netReward.toFixed(2)}`);
    setHudText(hud.penalty, `${penalty.toFixed(2)}`);
    setHudText(hud.mode, this.modeName);
    setHudText(hud.policy, "politica: fallback DQN visual");
    setHudText(hud.clearance, `clearance: ${scan.minDistance.toFixed(1)} m`);
    setHudText(hud.event, `${this.lastEvent}; WebGL no bloqueo la pantalla`);
    setHudText(hud.targetKind, scan.nearest ? scan.nearest.type : "sin target");
    setHudText(hud.targetId, scan.nearest ? `FB-${Math.round(scan.nearest.z)}` : "NEURODRIVER-SEARCH");
    setHudText(
      hud.targetDetail,
      scan.nearest ? `${scan.nearest.type} a ${scan.minDistance.toFixed(1)} m` : "buscando obstaculo"
    );
    setHudText(hud.targetZoom, `${lerp(1, 8, 1 - scan.normalized).toFixed(1)}x`);
    setHudText(hud.targetConfidence, `${Math.round((1 - scan.normalized) * 100)}%`);
    if (hud.targetLock) hud.targetLock.style.width = `${Math.round((1 - scan.normalized) * 100)}%`;
  ...
  updateQBars() {
    ...
    const min = Math.min(...this.lastQ);
    const max = Math.max(...this.lastQ);
    const span = Math.max(0.001, max - min);
    rows.forEach((row, index) => {
      const value = this.lastQ[index] || 0;
      const percent = ((value - min) / span) * 100;
      row.classList.toggle("active", index === this.lastAction);
      const fill = row.querySelector(".q-fill");
      const label = row.querySelector(".q-value");
      if (fill) fill.style.width = `${clamp(percent, 4, 100)}%`;
...
class DroneGame {
  constructor(modeName) {
    ...
    this.renderer.setAnimationLoop(() => {
      try {
        ...
        console.error(error);
        startCanvasFallback(this.modeName, error);
      }
    });
  }
  ...
  updateHud(netReward, penalty, scan) {
    setHudText(hud.distance, `${Math.max(0, this.drone.position.z - this.startZ).toFixed(0)} m`);
    setHudText(hud.altitude, `${this.drone.position.y.toFixed(1)} m`);
    setHudText(hud.speed, `${this.velocity.z.toFixed(1)} m/s`);
    setHudText(hud.episode, `${this.episode}`);
    setHudText(hud.reward, `${netReward.toFixed(2)}`);
    setHudText(hud.penalty, `${penalty.toFixed(2)}`);
    setHudText(hud.mode, this.modeName);
    setHudText(
      hud.policy,
      this.agent.exploring
      ? `politica: explorando e=${this.agent.epsilon.toFixed(2)}`
      : `politica: explotando e=${this.agent.epsilon.toFixed(2)}`
    );
    setHudText(hud.clearance, `clearance: ${scan.minDistance.toFixed(1)} m`);
    setHudText(hud.event, `${this.lightMode} | target ${this.scanSnapshot.kind}: ${this.lastEvent}`);
    ...
    if (hud.qBars) this.updateQBars();
    if (hud.rewardChart) this.drawRewardChart();
  }
