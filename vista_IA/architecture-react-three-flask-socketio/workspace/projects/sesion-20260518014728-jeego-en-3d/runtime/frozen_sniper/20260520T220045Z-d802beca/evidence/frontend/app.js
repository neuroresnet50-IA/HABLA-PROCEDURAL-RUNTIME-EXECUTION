...
function boot() {
  ...
  try {
    ...
    renderCanvasFallback(error, modeName);
    console.error(error);
  }
}

function renderCanvasFallback(error, modeName) {
  const canvas = document.querySelector("#world");
  if (!canvas) return;

  const context = canvas.getContext("2d");
  if (!context) {
    if (hud.event) hud.event.textContent = "error canvas: contexto 2d no disponible";
    return;
  }

  canvas.dataset.renderMode = "fallback-2d";
  const reason = error && error.message ? error.message : "WebGL no disponible";
  ...
  function drawFallback(now) {
    const { width, height } = resizeFallbackCanvas();
    ...
    sky.addColorStop(0, "#8fd1e8");
    sky.addColorStop(0.46, "#2b5356");
    sky.addColorStop(1, "#101615");
    ...
    context.beginPath();
    context.moveTo(centerX - 44, horizon);
    context.lineTo(centerX + 44, horizon);
    context.lineTo(width * 0.78, height);
    context.lineTo(width * 0.22, height);
    context.closePath();
    context.fill();

    context.strokeStyle = "rgba(222,240,222,0.55)";
    context.lineWidth = 2;
    for (let lane = 0; lane < 12; lane += 1) {
      const t = ((lane / 12) + elapsed * 0.22) % 1;
      const y = lerp(horizon + 12, height + 40, t);
      const spread = lerp(18, width * 0.19, t);
      context.beginPath();
      context.moveTo(centerX - spread * 0.08, y - 18);
      context.lineTo(centerX + spread * 0.08, y + 18);
      context.stroke();
    }

    for (let i = 0; i < 18; i += 1) {
      const t = ((i / 18) + elapsed * 0.08) % 1;
      const y = lerp(horizon + 8, ground, t);
      const depth = Math.max(0.12, t);
      const roadHalf = lerp(52, width * 0.31, t);
      const buildingWidth = lerp(22, 88, t);
      const buildingHeight = lerp(52, 240, t) * (i % 3 === 0 ? 1.25 : 1);
      const side = i % 2 === 0 ? -1 : 1;
      const x = centerX + side * (roadHalf + buildingWidth * 0.75);
      context.fillStyle = i % 3 === 0 ? "#314844" : "#3f342e";
      context.fillRect(x - buildingWidth / 2, y - buildingHeight, buildingWidth, buildingHeight);
      context.fillStyle = "rgba(255,224,128,0.58)";
      for (let w = 0; w < 4; w += 1) {
        const wx = x - buildingWidth * 0.32 + w * buildingWidth * 0.2;
        context.fillRect(wx, y - buildingHeight + 14 + (i % 4) * 8, Math.max(3, 5 * depth), Math.max(5, 9 * depth));
      }
    }

    const droneY = height * 0.46 + Math.sin(elapsed * 2.1) * 8;
    context.save();
    context.translate(centerX, droneY);
    context.fillStyle = "#dce9dd";
    context.strokeStyle = "#111817";
    context.lineWidth = 3;
    context.beginPath();
    context.moveTo(0, -18);
    ...
    context.lineWidth = 2;
    context.beginPath();
    context.arc(-34, -2, 12 + Math.sin(elapsed * 28) * 2, 0, Math.PI * 2);
    context.arc(34, -2, 12 - Math.sin(elapsed * 28) * 2, 0, Math.PI * 2);
    context.stroke();
    context.restore();

    const distance = Math.max(0, Math.floor(elapsed * 18));
    if (hud.distance) hud.distance.textContent = distance + " m";
    if (hud.altitude) hud.altitude.textContent = (18 + Math.sin(elapsed * 1.8) * 3).toFixed(1) + " m";
    if (hud.speed) hud.speed.textContent = "18.0 m/s";
    if (hud.mode) hud.mode.textContent = modeName;
    if (hud.policy) hud.policy.textContent = "politica: fallback visual";
    if (hud.clearance) hud.clearance.textContent = "clearance: fallback";
    if (hud.event) hud.event.textContent = "fallback canvas activo: " + reason;
...
class DroneGame {
  constructor(modeName) {
    ...
    if (!worldCanvas) throw new Error("Canvas #world no encontrado");
    this.renderer = new THREE.WebGLRenderer({
      canvas: worldCanvas,
      antialias: true,
      alpha: false
    });
    worldCanvas.dataset.renderMode = "webgl";
    ...
    this.chartContext = hud.rewardChart ? hud.rewardChart.getContext("2d") : null;
  ...
  updateHud(netReward, penalty, scan) {
    if (hud.distance) hud.distance.textContent = `${Math.max(0, this.drone.position.z - this.startZ).toFixed(0)} m`;
    if (hud.altitude) hud.altitude.textContent = `${this.drone.position.y.toFixed(1)} m`;
    if (hud.speed) hud.speed.textContent = `${this.velocity.z.toFixed(1)} m/s`;
    if (hud.episode) hud.episode.textContent = `${this.episode}`;
    if (hud.reward) hud.reward.textContent = `${netReward.toFixed(2)}`;
    if (hud.penalty) hud.penalty.textContent = `${penalty.toFixed(2)}`;
    if (hud.mode) hud.mode.textContent = this.modeName;
    if (hud.policy) {
    ...
    if (hud.clearance) hud.clearance.textContent = `clearance: ${scan.minDistance.toFixed(1)} m`;
    if (hud.event) hud.event.textContent = `${this.lightMode} | target ${this.scanSnapshot.kind}: ${this.lastEvent}`;
