...
TEMPORAL_PATTERNS = [
    # Datos cambiantes: población/habitantes, cargos actuales, precios, versiones, clima, noticias, fechas modernas.
...
class SemanticClassifier:
    """Clasificador semántico híbrido.

    1. Si existe un LLM con método generate(), lo usa como clasificador semántico real.
    2. Si el LLM falla o no existe, usa reglas ampliadas como fallback auditable.

    El LLM NO responde la pregunta; solo devuelve JSON de clasificación.
    """
...
""".strip()
        try:
            ...
            # El motor no debe caerse porque el clasificador LLM falle.
            return None
    ...
    def _classify_with_rules(self, state: HablaState) -> None:
        ...
        elif any(re.search(p, q) for p in TEMPORAL_PATTERNS):
            self._apply_type(state, "HECHO_TEMPORAL", "patrones de hecho temporal/ampliado", "rule_fallback")
        elif any(re.search(p, q) for p in OPINION_PATTERNS):
